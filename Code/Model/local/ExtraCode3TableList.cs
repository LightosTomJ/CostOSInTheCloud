...
namespace Model.local
{
	...
	/// </summary>
	[Serializable]
	public class ExtraCode3TableList : BaseTableList, Transferable
	{
		// private Vector _o_items;
		private ExtraCode3Table[] _o_items;
		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.extraCode3ListDataFlavor};

		private const long serialVersionUID = 1L;

		public ExtraCode3TableList(ExtraCode3Table[] items)
		{
			GroupCodes = items;
		}

		public virtual ExtraCode3Table[] GroupCodes
		{
			set
			{
				_o_items = value;
			...
			{
				return _o_items;
				//return (ExtraCode3Table[])_o_items.toArray(new ExtraCode3Table[_o_items.size()]);
			}
		}


		public virtual ExtraCode3Table[] GroupCodesCopy
		{
			get
			{
				ExtraCode3Table[] copy = new ExtraCode3Table[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
				   copy[i] = (ExtraCode3Table)_o_items[i].Clone();
				}
		...
		{
			return DataFlavors.extraCode3ListDataFlavor.Equals(flavor);
		}
...
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.extraCode3ListDataFlavor.Equals(flavor))
				{
					return this;
				}
