...
namespace Model.local
{
	...
	/// </summary>
	[Serializable]
	public class ExtraCode4TableList : BaseTableList, Transferable
	{
		// private Vector _o_items;
		private ExtraCode4Table[] _o_items;
		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.extraCode4ListDataFlavor};

		private const long serialVersionUID = 1L;

		public ExtraCode4TableList(ExtraCode4Table[] items)
		{
			GroupCodes = items;
		}

		public virtual ExtraCode4Table[] GroupCodes
		{
			set
			{
				_o_items = value;
			...
			{
				return _o_items;
				//return (ExtraCode4Table[])_o_items.toArray(new ExtraCode4Table[_o_items.size()]);
			}
		}


		public virtual ExtraCode4Table[] GroupCodesCopy
		{
			get
			{
				ExtraCode4Table[] copy = new ExtraCode4Table[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
				   copy[i] = (ExtraCode4Table)_o_items[i].Clone();
				}
		...
		{
			return DataFlavors.extraCode4ListDataFlavor.Equals(flavor);
		}
...
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.extraCode4ListDataFlavor.Equals(flavor))
				{
					return this;
				}
