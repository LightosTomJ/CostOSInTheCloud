using System;

/*
 * Created on 17 ��� 2006
 *
 * DataVers Technologies (c) 2005
 */
namespace Model.proj
{

	using BaseTable = nomitech.common.@base.BaseTable;
	using BaseTableList = nomitech.common.@base.BaseTableList;
	using DatabaseTable = nomitech.common.@base.DatabaseTable;
	using GroupCode = nomitech.common.@base.GroupCode;
	using AssemblyTable = nomitech.common.db.local.AssemblyTable;
	using AssemblyTableList = nomitech.common.db.local.AssemblyTableList;

	/// <summary>
	/// @author george
	/// 
	/// TODO To change the template for this generated type comment go to
	/// Window - Preferences - Java - Code Style - Code Templates
	/// </summary>
	[Serializable]
	public class BoqItemWithGroupCodeTableList : BaseTableList, Transferable
	{
		// private Vector _o_items;
		private BoqItemTable[] _o_items;
		private GroupCode[] groupCodes;
		private string groupToUse;
		private string currencyCode;

		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.boqitemWithGroupCodeListDataFlavor};

		private const long serialVersionUID = 1L;

		public BoqItemWithGroupCodeTableList(DatabaseTable[] items, GroupCode[] groupCodes, string groupToUse, string currencyCode)
		{
			BoqItems = items;
			this.groupCodes = groupCodes;
			this.groupToUse = groupToUse;
			this.currencyCode = currencyCode;
		}

		public BoqItemWithGroupCodeTableList(BoqItemTable[] items, GroupCode[] groupCodes, string groupToUse, string currencyCode)
		{
			BoqItems = items;
			this.groupCodes = groupCodes;
			this.groupToUse = groupToUse;
			this.currencyCode = currencyCode;
		}

		public virtual string GroupToUse
		{
			get
			{
				return groupToUse;
			}
		}

		public virtual BoqItemTable[] BoqItems
		{
			set
			{
				_o_items = value;
				//_o_items = new Vector(value.length);
				//for ( int i = 0; i < value.length; i++ ) {
				// _o_items.add(value[i]);
				//}
			}
			get
			{
				return _o_items;
				//return (BoqItemTable[])_o_items.toArray(new BoqItemTable[_o_items.size()]);
			}
		}

		public virtual DatabaseTable[] BoqItems
		{
			set
			{
				_o_items = new BoqItemTable[value.Length];
				for (int i = 0; i < value.Length; i++)
				{
					_o_items[i] = (BoqItemTable)value[i];
				}
			}
		}

		public virtual GroupCode[] GroupCodes
		{
			get
			{
				return groupCodes;
			}
		}


		public virtual BoqItemTable[] BoqItemsCopy
		{
			get
			{
				BoqItemTable[] copy = new BoqItemTable[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
					copy[i] = (BoqItemTable)_o_items[i].clone();
				}
    
				return copy;
			}
		}

		public virtual BoqItemTable[] getBoqItemsDeepCopy(bool ciclic)
		{
			BoqItemTable[] copy = new BoqItemTable[_o_items.Length];
			for (int i = 0; i < _o_items.Length; i++)
			{
				copy[i] = (BoqItemTable)_o_items[i].deepCopy(ciclic);
			}

			return copy;
		}

		public virtual DataFlavor[] TransferDataFlavors
		{
			get
			{
				lock (this)
				{
					return s_supportedDataFlavors;
				}
			}
		}

		public virtual bool isDataFlavorSupported(DataFlavor flavor)
		{
			return DataFlavors.boqitemWithGroupCodeListDataFlavor.Equals(flavor);
		}

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.boqitemWithGroupCodeListDataFlavor.Equals(flavor))
				{
					return this;
				}
        
				throw new UnsupportedFlavorException(flavor);
			}
		}

		public override BaseTable[] BaseTables
		{
			get
			{
				return BoqItems;
			}
		}

		public override BaseTable[] cloneBaseTables()
		{
			return getBoqItemsDeepCopy(true);
		}

		//	public AssemblyTableList convertToLineItemList(ProjectDBUtil prjDBUtil, boolean persistNew) {		
		//		return convertToLineItemList(prjDBUtil, persistNew, 100L);
		//	}

		public virtual AssemblyTableList convertToLineItemList(ProjectDBUtil prjDBUtil, bool persistNew, bool keepIds)
		{
			AssemblyTable[] items = new AssemblyTable[_o_items.Length];

			//System.out.println("Starting conversion...");
			for (int i = 0; i < _o_items.Length; i++)
			{
				items[i] = _o_items[i].convertToAssembly(prjDBUtil, currencyCode, persistNew, false, false, keepIds);
				if (!keepIds)
				{
					items[i].DatabaseCreationDate = 100L; // That is a missing definition ResourceUtil.MISSING
					items[i].DatabaseId = null;
				}

				if (items[i].ActivityBased.Value == false)
				{
					items[i].Productivity = decimal.One;
				}
				//System.out.println("I converted: "+items[i].getTitle()+" it now has "+items[i].getResourceAssignments(null).size()+" before "+
				//		_o_items[i].getResourceAssignments(null).size()
				//		);
			}

			return new AssemblyTableList(items);
		}
	}

}