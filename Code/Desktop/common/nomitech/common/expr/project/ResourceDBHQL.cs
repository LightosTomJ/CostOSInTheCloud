...
namespace Desktop.common.nomitech.common.expr.project
{
	...
	public class ResourceDBHQL : AbstractFunction
	{
...
//ORIGINAL LINE: public org.boris.expr.Expr evaluate(org.boris.expr.IEvaluationContext paramIEvaluationContext, org.boris.expr.Expr[] paramArrayOfExpr) throws org.boris.expr.ExprException
		...
		try
		{
		  ...
		  if (bool2)
		  {
			...
			for (sbyte b2 = 1; b2 < paramArrayOfExpr.Length; b2++)
			{
			  ...
			  {
				query = query.setString(b2 - true, ((ExprString)paramArrayOfExpr[b2]).str);
			  }
		  ...
		  else
		  {
			...
			list = DatabaseDBUtil.executeHqlQuery(str, arrayList);
			if (list.Count != 0)
			{
			  object @object = list[0];
			  if (@object is HqlResultValue[])
			  {
				i = (HqlResultValue[])@object.length;
			  }
			  else if (@object is object[])
			  {
				i = (object[])@object.length;
	  ...
	  private void setArrayValue(ExprArray paramExprArray, object paramObject, int paramInt1, int paramInt2)
	  {
		...
		{
		  paramExprArray.set(paramInt1, paramInt2, new ExprDouble(((int?)paramObject).Value));
		}
	...
	/* Location:              C:\Users\TomJames_zyl8law\Lightos\Lightos Hub - Documents\01-Clients\CostOS\Solution from Java Source Code\common\!\nomitech\common\expr\project\ResourceDBHQL.class
