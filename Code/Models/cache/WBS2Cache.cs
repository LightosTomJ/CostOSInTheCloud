...
namespace Models.cache
{
	...
	public class WBS2Cache : BaseCache
	{
		public const string TABLE_NAME = "ProjectWBS2Table";
		...
		public WBS2Cache(ProjectDBUtil prjDBUtil)
		{
		...
		public WBS2Cache(ProjectDBUtil prjDBUtil, bool initCaches)
		{
			o_prjDbUtil = prjDBUtil;
			if (initCaches)
			{
	...
	//	public WBS2Cache(String code, String name) {
	//		initCache(code,name);
	//	}
		...
		public virtual void remove(ProjectWBS2Table gcTable)
		{
			//System.out.println("Removing "+gcTable);
			hashMap.Remove(gcTable.GroupCode);
		}

		public virtual void add(ProjectWBS2Table gcTable)
		{
			//System.out.println("Adding "+gcTable);
			hashMap[gcTable.GroupCode] = (GroupCode)gcTable.clone();
		}

		public virtual void update(ProjectWBS2Table gcTable)
		{
			//System.out.println("Updating "+gcTable);
			hashMap[gcTable.GroupCode] = (GroupCode)gcTable.clone();
		}

		public virtual void addOrUpdate(ProjectWBS2Table obj)
		{
	...
	//		if ( groupCodeStyle.equals(ALPHABETIC_GC_STYLE)) {
	//			isAlphabetic = true;
	//		}
...
//ORIGINAL LINE: public void initializeCache() throws Exception
		public virtual void initializeCache()
		{
			...
			IEnumerator<ProjectWBS2Table> iter = session.createQuery("from ProjectWBS2Table as o where o.projectId = " + BaseDBUtil.ProjectUrlId).list().GetEnumerator();

			while (iter.MoveNext())
			{
				ProjectWBS2Table groupCodeTable = (ProjectWBS2Table)iter.Current;
			...
			if ( prj != null ) {
				...
				Iterator iter1 = prj.getWbs2Set().iterator();
	
				//System.out.println("Processing: "+prj);
				while ( iter1.hasNext() ) {
					ProjectWBS2Table groupCodeTable = (ProjectWBS2Table)iter1.next();
					//System.out.println("Adding: "+groupCodeTable);
		...
		{
			private readonly WBS2Cache outerInstance;


			internal GroupCode code;
			internal bool alwaysShowGroupPrefix;

			internal WBSTreeNode(WBS2Cache outerInstance, GroupCode code, bool alwaysShowGroupPrefix)
			{
	...
	//			String separator = GroupCodeUtil.getCodeToCodeSeparator("wbs2Code");
	//			String titleSeparator = GroupCodeUtil.getCodeToTitleSeparator("wbs2Code");
