...
namespace Models.local
{
	...
	/// </summary>
	[Serializable]
	public class ExtraCode9TableList : BaseTableList, Transferable
	{
		// private Vector _o_items;
		private ExtraCode9Table[] _o_items;
		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.extraCode9ListDataFlavor};

		private const long serialVersionUID = 1L;

		public ExtraCode9TableList(ExtraCode9Table[] items)
		{
			GroupCodes = items;
		}

		public virtual ExtraCode9Table[] GroupCodes
		{
			set
			{
				_o_items = value;
			...
			{
				return _o_items;
				//return (ExtraCode9Table[])_o_items.toArray(new ExtraCode9Table[_o_items.size()]);
			}
		}


		public virtual ExtraCode9Table[] GroupCodesCopy
		{
			get
			{
				ExtraCode9Table[] copy = new ExtraCode9Table[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
				   copy[i] = (ExtraCode9Table)_o_items[i].Clone();
				}
		...
		{
			return DataFlavors.extraCode9ListDataFlavor.Equals(flavor);
		}
...
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.extraCode9ListDataFlavor.Equals(flavor))
				{
					return this;
				}
