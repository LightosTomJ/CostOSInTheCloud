...
namespace Models.local
{
	...
	/// </summary>
	[Serializable]
	public class SupplierTableList : Transferable, BaseTableList
	{
		// private Vector _o_items;
		private SupplierTable[] _o_items;
		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.supplierListDataFlavor};

		private const long serialVersionUID = 1L;

		public SupplierTableList(SupplierTable[] items)
		{
			Suppliers = items;
		}

		public virtual SupplierTable[] Suppliers
		{
			set
			{
				_o_items = value;
			...
			{
				return _o_items;
				//return (SupplierTable[])_o_items.toArray(new SupplierTable[_o_items.size()]);
			}
		}


		public virtual SupplierTable[] SuppliersCopy
		{
			get
			{
				SupplierTable[] copy = new SupplierTable[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
				   copy[i] = (SupplierTable)_o_items[i].clone();
				}
...
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.supplierListDataFlavor.Equals(flavor))
				{
					return this;
				}
