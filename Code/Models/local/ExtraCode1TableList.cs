...
namespace Models.local
{
	...
	/// </summary>
	[Serializable]
	public class ExtraCode1TableList : BaseTableList, Transferable
	{
		// private Vector _o_items;
		private ExtraCode1Table[] _o_items;
		private static readonly DataFlavor[] s_supportedDataFlavors = new DataFlavor[] {DataFlavors.extraCode1ListDataFlavor};

		private const long serialVersionUID = 1L;

		public ExtraCode1TableList(ExtraCode1Table[] items)
		{
			GroupCodes = items;
		}

		public virtual ExtraCode1Table[] GroupCodes
		{
			set
			{
				_o_items = value;
			...
			{
				return _o_items;
				//return (ExtraCode1Table[])_o_items.toArray(new ExtraCode1Table[_o_items.size()]);
			}
		}


		public virtual ExtraCode1Table[] GroupCodesCopy
		{
			get
			{
				ExtraCode1Table[] copy = new ExtraCode1Table[_o_items.Length];
				for (int i = 0; i < _o_items.Length; i++)
				{
				   copy[i] = (ExtraCode1Table)_o_items[i].clone();
				}
		...
		{
			return DataFlavors.extraCode1ListDataFlavor.Equals(flavor);
		}
...
//ORIGINAL LINE: public synchronized Object getTransferData(java.awt.datatransfer.DataFlavor flavor) throws java.awt.datatransfer.UnsupportedFlavorException, java.io.IOException
		public virtual object getTransferData(DataFlavor flavor)
		{
			lock (this)
			{
				if (DataFlavors.extraCode1ListDataFlavor.Equals(flavor))
				{
					return this;
				}
